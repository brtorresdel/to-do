...
import type { taskFormCss } from "../types/cssDecoration";
import { ThemeContext, type ThemeContextType } from "../context/Theme";
import { useContext } from "react";

export default function TaskForm ({ taskID }: { taskID: TaskInfoProps }) {
    ...
    return (
        <div className={`w-1/2 h-fit rounded-md opacity-100 p-5 flex flex-col shadow-mg gap-2 ${taskFormCss.bg}`}>
            ...
            <form className="w-full flex flex-col gap-2">
                <InputElem name="título" required={true} theme={taskFormCss}/>
                <InputElem name="prazo" required={false} theme={taskFormCss}/>
                <InputElem name="descrição" required={true} theme={taskFormCss}/>
                <InputElem name="observações" required={false} theme={taskFormCss}/>
                <div>
...
function InputElem ( {name, required, value, theme}: {name: string, required: boolean, value?: string, theme: taskFormCss} ) {
