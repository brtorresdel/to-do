        ...
        hidden ? <Modal>
            <div className={`w-1/2 h-fit rounded-md opacity-100 p-5 flex flex-col shadow-mg gap-2 ${taskFormCss.bg}`}>
                ...
                <form className="w-full flex flex-col gap-2">
                    <InputElem name="título *" required={true} theme={theme} value={title} onChange={value => setTitle(value)} submitTry={submitTry}/>
                    <InputElem name="prazo" required={false}  theme={theme} value={dueDate} onChange={value => setDueDate(value)} submitTry={submitTry}/>
                    <InputElem name="descrição *" required={true} theme={theme} value={description} onChange={value => setDescription(value)} submitTry={submitTry}/>
                    <InputElem name="observações" required={false} theme={theme} value={observations} onChange={value => setObservations(value)} submitTry={submitTry}/>
                    <div>
                        <button 
...
function InputElem ( {name, required, value, theme, submitTry, onChange}: {name: string, required: boolean, value?: string, theme: "light" | "dark", submitTry: boolean, onChange: (value: string) => void} ) {
    // TODO: add the required function to validade the data entry
    
    const capitalize = name.charAt(0).toUpperCase() + name.slice(1);
    const textArea = name === "descrição" || name === "observações";

    const showRequired = required && submitTry && (!value || value.trim() === "");
    ...
    return <div className="w-full mb-1 mt-1">
        <div className="flex flex-row gap-2 items-center">
            <label htmlFor={name} 
            className={`font-semibold ${inputCss.text}`}>{capitalize}</label>
            {required && <span className={`text-red-400 font-bold text-sm ${showRequired ? "" : "hidden"}`}>Este campo é obrigatório!</span>}
        </div>
        {textArea ?
            <textarea
                name={name}
